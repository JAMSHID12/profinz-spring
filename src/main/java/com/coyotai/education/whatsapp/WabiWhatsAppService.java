...
/**
 * WABI integration API: the API key selects the approved template; external_id
...
public class WabiWhatsAppService implements WhatsAppService {
	...
	public WabiWhatsAppService(RestTemplate client, WhatsAppProperties properties) {
		...
		if (!"https".equalsIgnoreCase(base.getScheme()) || base.getHost() == null || base.getUserInfo() != null
			...
			throw new IllegalStateException(
					"WABI_BASE_URL must be an HTTPS URL without credentials, query or fragment");
	...
	@Override
	public WhatsAppSendResult send(String to, NotificationPayload payload) {
		return send(to, payload, properties.wabi().event(), "education-test-" + UUID.randomUUID());
	}

	@Override
	public WhatsAppSendResult send(String to, NotificationPayload payload, NotificationEvent event, String externalId) {
		String apiKey = properties.wabi().keyFor(event);
		if (apiKey == null || apiKey.isBlank())
			return WhatsAppSendResult.failure("No WABI key configured for " + event);
		if (payload == null) return WhatsAppSendResult.failure("Missing notification payload");
		if (!Objects.equals(payload.templateName(), properties.templateName(event)))
			return WhatsAppSendResult.failure(
					"Queued template does not match the configured WABI template. Create a notification with the current configuration.");
		String number = PhoneNumbers.toWhatsAppFormat(to);
		if (!PhoneNumbers.isValid(to) || number == null || !number.matches("[1-9][0-9]{6,14}"))
		...
		if ((event == NotificationEvent.STUDENT_LATE || event == NotificationEvent.STUDENT_ABSENT) && (payload.parameters().size() != 5
				|| payload.parameters().stream().anyMatch(v -> v == null || v.isBlank())))
			return WhatsAppSendResult
					.failure("Attendance template requires five non-empty parameters: parent, student, class, date, centre");
		...
		if (!payload.parameters().isEmpty())
			body.put("contact_name", payload.parameters().get(0));
		try {
			ResponseEntity<JsonNode> response = client.exchange(endpoint, HttpMethod.POST,
			...
			if (id == null)
				id = result.path("message_id").asText(externalId);
			return WhatsAppSendResult.success(id);
		} catch (HttpStatusCodeException ex) {
			if (ex.getStatusCode().value() == 409)
				return WhatsAppSendResult.failure(
						"WABI HTTP 409: this external_id already belongs to different content. Retry only unchanged content; create a new notification for new content.");
			// Never include the request headers, API key or raw response in logs/errors.
			return WhatsAppSendResult.failure("WABI HTTP " + ex.getStatusCode().value()
					+ ": check key, template approval, opt-in and parameter order");
		} catch (RestClientException ex) {
			return WhatsAppSendResult
					.failure("WABI unavailable; the notification can be retried with the same external ID");
		}
	}

	private String safe(String error) {
		String clean = error;
        if (properties.wabi().apiKey() != null && !properties.wabi().apiKey().isBlank())
            clean = clean.replace(properties.wabi().apiKey(), "[redacted]");
        if (properties.wabi().eventApiKeys() != null) {
            for (String key : properties.wabi().eventApiKeys().values())
                if (key != null && !key.isBlank()) clean = clean.replace(key, "[redacted]");
        }
